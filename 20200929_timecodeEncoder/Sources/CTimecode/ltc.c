...
int decodeLocalFile(FILE *file) {
	...
	do {
		n = fread(sound, sizeof(ltcsnd_sample_t), BUFFER_SIZE, file);
...
int encodeAndSaveToLocal(FILE *file){
    
        double length = 30; // in seconds
        double fps = 25;
        ...
        st.years =  timeinfo->tm_year;
        st.months = timeinfo->tm_mon + 1;
        st.days = timeinfo->tm_mday;

        st.hours = timeinfo->tm_hour;
        st.mins = timeinfo->tm_min;
        st.secs = timeinfo->tm_sec;
        st.frame = 0;
